...
from ..indicators import macd as compute_macd, market_regime_ok, rate_of_change, rsi, sma as compute_sma
...
class OpportunisticStrategy:
    ...
    def evaluate_symbol(self, symbol: str, regime_ok: bool = True, force: bool = False, cache_only: bool = False) -> SignalResult | None:
        ...
        # Gate 2: RSI subiendo N días consecutivos.
        # rsi_rising_min_days=0: all([]) = True vacuamente (gate desactivado).
        n_rsi = opp.rsi_rising_min_days
        if len(rsi_short) >= n_rsi + 1:
            rsi_rising_ok = all(
                rsi_short.iloc[-(k + 1)] < rsi_short.iloc[-k]
                for k in range(1, n_rsi + 1)
            )
        else:
            rsi_rising_ok = n_rsi == 0

        # Gate 3: ATR máximo duro. Nombres con ATR >N% tienen stop de 5% < 1×ATR.
        volatility_max_ok = volatility_pct <= opp.max_volatility_pct_gate

        # Gate 4: No entrar tarde en un rally ya corrido.
        roc_5d = rate_of_change(close, 5)
        last_roc_5d = float(roc_5d.iloc[-1]) if not roc_5d.empty and not pd.isna(roc_5d.iloc[-1]) else 0.0
        run_ok = last_roc_5d <= opp.max_5d_run_pct

        # Gate 5: Precio por encima de SMA(N). Period=0 deshabilita el gate.
        if opp.require_above_sma_period > 0:
            sma_s = compute_sma(close, opp.require_above_sma_period)
            last_sma = float(sma_s.iloc[-1]) if not sma_s.empty and not pd.isna(sma_s.iloc[-1]) else 0.0
            above_sma_ok = last_price > last_sma if last_sma > 0 else False
        else:
            above_sma_ok = True
        ...
        earnings_ok = True
        all_quality_gates_ok = (
            momentum_ok and rsi_ok and volatility_ok and liquidity_ok and room_to_grow_ok
            and regime_ok and macd_crossover_ok and rsi_rising_ok and volatility_max_ok
            and run_ok and above_sma_ok
        )
        if all_quality_gates_ok:
        ...
        if not volatility_max_ok:
            notes.append(f"Volatilidad {volatility_pct:.1f}% (ATR/precio) por encima del techo ({opp.max_volatility_pct_gate}%): demasiado erratico para el stop de 5%.")
        if not liquidity_ok:
        ...
        if not macd_crossover_ok:
            notes.append(f"MACD no cruzó a positivo en los últimos {opp.macd_crossover_lookback_days} días: sin inflection point reciente.")
        if not rsi_rising_ok:
            notes.append(f"RSI no lleva {opp.rsi_rising_min_days} días consecutivos subiendo: recuperacion no confirmada.")
        if not run_ok:
            notes.append(f"Rally de 5 días de {last_roc_5d:.1f}% supera el máximo ({opp.max_5d_run_pct}%): entrada tardía.")
        if not above_sma_ok:
            notes.append(f"Precio por debajo de SMA({opp.require_above_sma_period}): giro todavía no establecido como tendencia.")
        if not earnings_ok:
        ...
        return SignalResult(
            ...
            passes_filters=(
                momentum_ok and rsi_ok and volatility_ok and volatility_max_ok
                and liquidity_ok and room_to_grow_ok and regime_ok
                and macd_crossover_ok and rsi_rising_ok and run_ok and above_sma_ok
                and earnings_ok
            ),
